...
if (isset($_GET['item'])) {
	...
	if ($item > 1) {
	  echo '<a href="?text_literary&item='.($item-1).$langPostfix.'">&lt;&lt;&nbsp;Předchozí položka</a>&nbsp;&nbsp;';
	}
	if ($item < $count) {
	  echo '<a href="?text_literary&item='.($item+1).$langPostfix.'">Následující položka&nbsp;&gt;&gt;</a>';
    }
} else if ($count == 0) {
	...
	for ($i = $fst; $i > $pagepos; $i--) {
      ...
      echo '<li>';
      echo '<p><strong><a href="?text_literary&item='.$i.$langPostfix.'">'.$title.'</a></strong>'.$notice.'<br/>Vytvořeno '.date('d. m. Y H:m:s', $time).'<br/>'.$shorttext.'</p>';
      echo "</li>\n";
	}
    echo '</ul>';
    echo "\n";
	...
	if ($count > $perpage) {
	  echo '<p>';
	  if ($pos > 0) {
		echo '<a href="?text_literary&pos='.($pos-1).$langPostfix.'">&lt;&lt;&nbsp;Předchozí stránka</a>&nbsp;&nbsp;';
	  }
	  $maxpos = intdiv($count + $perpage - 1, $perpage) - 1; 
	  if ($pos < $maxpos) {
		echo '<a href="?text_literary&pos='.($pos+1).$langPostfix.'">Následující stránka&nbsp;&gt;&gt;</a>';
	  }
	}
} ?>
